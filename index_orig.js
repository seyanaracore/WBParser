  ...
  for (let i = 1; i <= urls.length; i++) {
    ...
    if (
      fetchedData.some((productData) =>
        productData.codes.includes(url)
      )
    ) {
      continue;
    }
    ...
    const productData = await page.evaluate(async () => {
		...
		document.querySelectorAll(".swiper-wrapper > li > a.img-plug").forEach((link) => {
			if(link.href) {
				window.productData.codes.push(link.href)
				let imageSrc = link.childNodes[1].src
				imageSrc = imageSrc.replace(imageSrc.split("/")[3],"big")
				let imageFormat = imageSrc.split(".")[imageSrc.split(".").length-1]
...
async function getProductsLinksList(sellerUrl, fileStream) {
  ...
  for (let i = initialIterationPage || 1; i < 999; i++) {
	...
	if(sellerUrl.at(-1) === "/") sellerUrl = sellerUrl.slice(0, sellerUrl.length-1)
...
const sellerName =
  settings.sellerUrl.split("seller/")[1]?.split("/")[0]?.split("?")[0] || sellerUrl.split("brands/")[1]?.split("/")[0]?.split("?")[0] || "WBGlobal";
const fetchedData = [];
...
const productsDataParams = {
  ...
  dataHandler: (product) => {
    ...
    for (let i = 0; i < product.codes.length; i++) {
      handledData += `,${product.codes[i]},${product.images[i]}${os.EOL}`;
    }
